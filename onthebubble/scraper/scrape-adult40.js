 ...
 * scrape-adult40.js
 *
 * Scrapes the 2025 NEOTA Adult 40 & Over (4.0 Men) flight from TennisRecord.
 * Phase 1 — Teams: seed all teams in the flight, parse rosters.
 * Phase 2 — Matches: collect match IDs, fetch court-by-court results.
 *
 * Output: data/adult40-scraped.json  { teams: [...], matches: [...] }
 */
...
const YEAR     = '2025';
const DELAY_MS = 700;
const OUT_FILE = path.join(__dirname, '../data/adult40-scraped.json');

const SEED_URLS = [
  `/adult/teamprofile.aspx?teamname=NEO%2fSmith%2fA40%2f4.0M&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20PM2%20Schlak&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20MM%20Burnoski&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20PW%20Plymak&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20NC%20Otto&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20AO%20Wissman&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20WR%20Krenn&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20WE%20Siebenschuh&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20PM1%20Harmon&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20CRC%20Budgen&year=${YEAR}`,
  `/adult/teamprofile.aspx?teamname=A40%204.0M%20MV1%20Gau&year=${YEAR}`,
];
...
async function scrapeTeamProfile(url) {
  ...
  $('table').each((i, tbl) => {
    ...
    if (/Adult 40\+|Mixed|Combo|Tri-Level/i.test(r0) && r0.length < 80) {
      const m = r0.match(/^(Adult \d+\+|Mixed \d+\+|Combo|Tri-Level)\s+(\S+)\s+(\S+)\s+(\S+)/i);
  ...
  $('table').each((i, tbl) => {
    ...
    $(tbl).find('tr').slice(1).each((j, tr) => {
      ...
      // For 8-col desktop table: col 3=2025Record, col 7=Rating
      // For 6-col mobile table:  col 3=2025Record, col 5=Rating
      const recCol    = 3;
      const ratingCol = colCount - 1;

      // Gender: Adult 40+ Men = 'M'
      const genderVal = gender || 'M';
      const rec       = parseRecord($(cells.eq(recCol)).text().trim());
      const dynRaw    = parseFloat($(cells.eq(ratingCol)).text().trim());
      const rating    = isNaN(dynRaw) ? null : dynRaw;

      players.push({
        name, location: loc, gender: genderVal, ntrp,
  ...
  return { url, teamName, leagueName, leagueType, section, gender, level, players, schedule };
}
...
async function main() {
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('  OnTheBubble — Adult 40+ (4.0 Men) Scraper');
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  for (const url of SEED_URLS) {
    ...
    if (!result) {
      ...
      console.log(`✓  ${result.players.length} players | ${result.leagueType} ${result.level}`);
      teams.push(result);
